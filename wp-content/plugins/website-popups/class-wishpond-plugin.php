    ...
    class WishpondPlugin {
        ...
        if (isset($_POST['submit']) && $_POST['submit']) {
          if (isset($_POST['token'])) {
            ...
            if ($token === get_option('wishpond_auth_token')) {
              $error_message = __('Token value is the same: ', 'website-popups') . $token;
            } else {
              $saved = update_option('wishpond_auth_token', $token, '', 'no');

              if ($saved) {
                $notice_message = __('Successfully updated token: ', 'website-popups') . $token;
              } else {
                $error_message = __('Unable to update token: ', 'website-popups') . $token;
              }
            }
          }
        }
      ...
      public function publish_campaign($data) {
        if (!is_user_logged_in()) {
          wp_die(__('Not logged in', 'website-popups'));
        }
        ...
        if (!WishpondUtilities::permalink_structure_valid()) {
          $message = __('Invalid permalink structure. Please go to "Settings"->"Permalinks" and make sure your permalinks use the "postname"', 'website-popups');
          ...
          $action   = __('saved', 'website-popups');

          if ($campaign->post_id !== -1) {
            $saved  = $campaign->update($path);
            $action = __('updated', 'website-popups');
          } else {
            $saved = $campaign->save();
          }
          ...
          if ($saved) {
            ...
            $message = WishpondUtilities::json_message('updated', __('Campaign ', 'website-popups') . $action . __(' successfully. ', 'website-popups') . $campaign_url);
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'website-popups'));
          }
        }
      ...
      public function delete_campaign($data) {
        ...
        if ($campaign) {
          if ($campaign->remove()) {
            $message = WishpondUtilities::json_message('updated', __('Campaign removed successfully.', 'website-popups'));
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'website-popups'));
          }
        } else {
          $message = WishpondUtilities::json_message('error', __('Campaign not found.', 'website-popups'));
        }
      ...
      function wishpond_load_plugin_textdomain() {
        load_plugin_textdomain( 'website-popups', FALSE, basename( dirname( __FILE__ ) ) . '/lang/' );
      }
    }
  }
