...
function testimonial_add_thumb_column( $columns ) {
    return array_merge( $columns, 
        array( 'thumb' => __( 'Featured Image', 'testimonial' ) ) );
}
add_filter( 'manage_testimonial_posts_columns' , 'testimonial_add_thumb_column' );




function testimonial_posts_thumb_display( $column, $post_id ) {
    if ($column == 'thumb'){
		
		$testimonial_thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post_id), 'thumbnail' );
		$testimonial_thumb_url = $testimonial_thumb['0'];
        echo '<img width="40px" height="40px" src="'.$testimonial_thumb_url.'">';
    }
}
add_action( 'manage_testimonial_posts_custom_column' , 'testimonial_posts_thumb_display', 10, 2 );
